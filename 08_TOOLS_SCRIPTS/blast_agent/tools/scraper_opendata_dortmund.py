    ...
    if isinstance(datasets_list, list):
        for ds in datasets_list:
            if isinstance(ds, dict):
                metas = ds.get("metas", {})
                default_meta = (
                    metas.get("default", {}) if isinstance(metas, dict) else {}
                )
                ...
                results.append(
                    {
                        ...
                        "records_count": (
                            default_meta.get("records_count")
                            if isinstance(default_meta, dict)
                            else None
                        ),
                    }
...
def main() -> None:
    if len(sys.argv) < 3:
        print(
            "Usage: python scraper_opendata_dortmund.py <search_datasets|get_records> <query|dataset_id>",
            file=sys.stderr,
        )
        sys.exit(1)
